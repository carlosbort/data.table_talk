...
(2) Basics data.table. 
    ...
    - The **i,j,by** syntax. + Exercises
  
(3) Intermediate data.table. 

    - .SD, multiple assign, setkey, setnames, %between%, .GRP, .BY, + Exercises.
  
(4) Advanced data.table and further steps 

    - Shift, set, .EACHI, margarittr,  merge, pattern: like, + Exercises. 
    - Others. fread(), melt(), dcast(), IDateTime(), fwrite() 
    

## Objective

- Data.table novice: Understand the package logic and its possibilities.

- Data.table pr0s: Get some useful tips.
...
<img src="https://media.giphy.com/media/Ma9C5ZLpqmJWM/giphy.gif" height="420" width="620">
...
<img src="https://media.giphy.com/media/mpZnI11wu4wx2/giphy.gif" height="420" width="680">
...
- Exercises with flights dataset
...
2.- How many flights went to Miami? (i=(condition subset), j= count)

```{r,echo=FALSE}
flights[dest=="MIA",.N]
```

3.- Calculate the total days of `air_time` (has NA's). (pssst: lubridate:::seconds_to_period()) (j= sum, airtime)
...
4.- Calculate the mean of `dep_delay` by al the diferent origin airports.

```{r,echo=F,warning=F,message=F}
flights[,mean(dep_delay,na.rm=T),by=origin]
```

5.- Create a new data.table `delays_month`. This data.table must contains the mean of departure delays, sd of departure delays, and the number of flights, aggregated by month. (Same column names!) 
...
6.- Create a new binary column named `not_delayed` when dep_delay <0 , equals 1. (psssst: ifelse() or subset and rewrite). 

```{r,echo=F}
flights[,not_delayed:=ifelse(dep_delay<0,1,0)]
```

7.- Calculate the mean of not delayed flights by company and its number. Compute the mean of the previous created column `not_delayed` and name the new as `perc_not_delayed`! Use the chaining to order by `perc_not_delayed`. (Add .N column)
...
<img src="https://media.giphy.com/media/PgwAgf7fbMrsY/giphy.gif" height="420" width="680">
...
# Data.table/Intermediate_Exercise

0.- Rename `not_delayed` column to `i_hate_delays`

```{r,echo=F}
setnames( flights, c("not_delayed"),c("i_hate_delays"))
colnames(flights)
setnames( flights, c("i_hate_delays"),c("not_delayed"))
```
